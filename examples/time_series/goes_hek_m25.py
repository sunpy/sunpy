...
flares_hek = client.search(
    hek.attrs.Time(tr.start, tr.end), hek.attrs.FL, hek.attrs.FRM.Name == "SWPC"
)
...
ax.axvspan(
    parse_time(flares_hek[0].get("event_starttime")).plot_date,
    parse_time(flares_hek[0].get("event_endtime")).plot_date,
    alpha=0.2,
    label=flares_hek[0].get("fl_goescls"),
)
ax.legend(loc=2)
ax.set_yscale("log")
plt.show()
