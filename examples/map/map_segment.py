...
###############################################################################
# Let's then segment the data based on coordinates and convert them to pixel
# index. We convert the segmented pixel values to 0.
...
new_data = np.where(new_data != True, np.nan, new_data)
...
ofsetted_map = sunpy.map.Map(new_data * smap.data, smap.meta)
fig = plt.figure()
fig.add_subplot(projection=smap)
ofsetted_map.plot()
plt.show()
...
rotated_map = sunpy.map.Map(new_data * out_warp.data, out_warp.meta)
