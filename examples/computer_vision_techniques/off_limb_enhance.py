...
y = np.array(
    [aia.data[(r > this_r) * (r < this_r + rsun_step_size)].mean() for this_r in rsun_array]
)
...
scaled_map.plot_settings["norm"] = ImageNormalize(
    stretch=aia.plot_settings["norm"].stretch, vmin=aia.data.min(), vmax=aia.data.max()
)
