...
# Earth.
stereo = (
    a.vso.Source("STEREO_B") & a.Instrument("EUVI") & a.Time("2011-01-01", "2011-01-01T00:10:00")
)

aia = a.Instrument("AIA") & a.vso.Sample(24 * u.hour) & a.Time("2011-01-01", "2011-01-02")
...
maps = {
    m.detector: m.submap(
        SkyCoord([-1100, 1100] * u.arcsec, [-1100, 1100] * u.arcsec, frame=m.coordinate_frame)
    )
...
aia_bottom_left = SkyCoord([[-800, -300]] * u.arcsec, frame=maps["AIA"].coordinate_frame)
aia_top_right = SkyCoord(
    aia_bottom_left.Tx + aia_width,
...
corners = (
    [aia_bottom_left.Tx, aia_bottom_left.Ty],
...
fig = plt.figure(figsize=(10, 4))
for i, (m, coord) in enumerate(zip([maps["EUVI"], maps["AIA"]], [hpc_B, hpc_aia])):
    ax = fig.add_subplot(1, 2, i + 1, projection=m)
    m.plot(axes=ax)
    ...
    m.draw_rectangle(coord[0], w, h, transform=ax.get_transform("world"))
