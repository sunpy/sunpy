...
This works by providing the URL of a remote file and a SHA256 hash to the `~sunpy.data.manager.require` decorator which can be added to functions that require these specific data files from remote sources.
...
See :ref:`remote-data` for more details.
...
`~sunpy.map.SUVIMap` provides SunPy map support for SUVI FITS image data, and the `~sunpy.net.sources.goes.SUVIClient` adds support for search SUVI data directly from NOAA via `Fido <sunpy.net.fido_factory.UnifiedDownloader>`.
