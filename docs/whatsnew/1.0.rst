...
- **Improved tools to get positions of bodies in the solar system**
  ...
  There is a new function `~sunpy.coordinates.ephemeris.get_horizons_coord` that queries `JPL HORIZONS <https://ssd.jpl.nasa.gov/?horizons>`_ for the location of solar-system bodies.
  JPL HORIZONS includes not only planets and other natural bodies in the solar system, but also major spacecraft.
  This function requires the `Astroquery <https://astroquery.readthedocs.io/en/latest/>`_ package and an Internet connection.
