...
Since :func:`scipy.ndimage.affine_transform` has more consistent results between biquartic interpolation (``order=4``) and bicubic interpolation (``order=3``), we have changed the default interpolation order for :meth:`sunpy.map.GenericMap.rotate` to bicubic interpolation for improved performance under default settings.
:func:`sunpy.image.transform.affine_transform` has always defaulted to bicubic interpolation.
