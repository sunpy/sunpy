...
Map data and units
==================
`~sunpy.map.Map` instances now have the ``.unit`` set to the correct `~astropy.units.Unit` if the 'BUNIT' keyword is set in the FITS metadata.
...
In addition, `sunpy.map.GenericMap.wcs` is now cached when the map metadata remains unchanged, significantly improving performance in applications that make multiple requests for the map WCS (e.g., plotting), and reducing the number of repeated warnings thrown when metadata is missing.
