...
In theory Python version does not change the results since we have pinned the packages that can cause the hash to vary.
...
The filename might have changed since the version of astropy or matplotlib or freetype gets updated.
