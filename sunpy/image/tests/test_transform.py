...
@pytest.mark.parametrize(
    "angle, k", [(90.0, 1), (-90.0, -1), (-270.0, 1), (-90.0, 3), (360.0, 0), (-360.0, 0)]
)
...
@pytest.mark.parametrize(
    "angle, dx, dy, scale_factor",
    [(90, -100, 50, 0.25), (-90, 50, -100, 0.75), (180, 100, 50, 1.5)],
)
def test_all(angle, dx, dy, scale_factor):
    ...
    expected = shift
    rotscaleshift = affine_transform(
        original, rmatrix=rmatrix, scale=scale_factor, recenter=True, image_center=rcen
    )
    ...
    transformed = affine_transform(
        original, rmatrix=rmatrix, scale=1.0, recenter=True, image_center=rcen
    )
    rcen = image_center - np.dot(rmatrix, np.array([dx, dy]))
    dx, dy = np.asarray(np.dot(rmatrix, disp), dtype=int)
    rmatrix = np.array([[c, s], [-s, c]])
    inverse = affine_transform(
        transformed, rmatrix=rmatrix, scale=1.0, recenter=True, image_center=rcen
    )
...
def test_nan_skimage_high(identity):
    ...
    in_arr = np.array([[np.nan]])
    with pytest.warns(
        SunpyUserWarning, match="Setting NaNs to 0 for higher-order scikit-image rotation."
    ):
