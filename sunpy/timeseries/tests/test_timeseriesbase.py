...
@pytest.fixture
def noaa_sxr_json_test_ts():
    return sunpy.timeseries.TimeSeries(noaa_goes_xrs_json_filepath_1d, source='noaagoessxr')
...
def test_units_type(eve_test_ts, esp_test_ts, fermi_gbm_test_ts, norh_test_ts, goes_test_ts,
    ...
    assert isinstance(noaa_sxr_json_test_ts.units, OrderedDict)
...
def test_meta_type(eve_test_ts, esp_test_ts, fermi_gbm_test_ts, norh_test_ts, goes_test_ts,
    ...
    assert isinstance(noaa_sxr_json_test_ts.meta, TimeSeriesMetaData)
...
def test_data_type(eve_test_ts, esp_test_ts, fermi_gbm_test_ts, norh_test_ts, goes_test_ts,
    ...
    assert isinstance(noaa_sxr_json_test_ts.to_dataframe(), DataFrame)
...
def test_noaa_sxr_json_invalid_peek(noaa_sxr_json_test_ts):
    a = noaa_sxr_json_test_ts.time_range.start - TimeDelta(2*u.day)
    b = noaa_sxr_json_test_ts.time_range.start - TimeDelta(1*u.day)
    empty_ts = noaa_sxr_json_test_ts.truncate(TimeRange(a, b))
    with pytest.raises(ValueError):
        empty_ts.peek()
