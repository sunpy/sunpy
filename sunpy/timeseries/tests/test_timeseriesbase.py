...
def test_truncation_slices(eve_test_ts, truncation_slice_test_ts_1, truncation_slice_test_ts_2):
    ...
    assert len(eve_test_ts.data) == (
        len(truncation_slice_test_ts_1.data) + len(truncation_slice_test_ts_2.data)
    )
    ...
    assert (
        truncation_slice_test_ts_1.time_range.start
        == truncation_slice_test_ts_1.meta.time_range.start
        == eve_test_ts.time_range.start
    )
    assert (
        truncation_slice_test_ts_2.time_range.end
...
def test_truncation_timerange(eve_test_ts, truncation_timerange_test_ts):
    ...
    assert (
        truncation_timerange_test_ts.time_range
...
def test_truncation_dates(eve_test_ts, truncation_dates_test_ts):
    # Check the resulting timerange in both TS and TSMD
    assert (
        truncation_dates_test_ts.time_range
...
def test_truncated_start_ts(concatenate_multi_files_ts, truncated_start_ts):
    ...
    assert (
        concatenate_multi_files_ts.meta.metadata[1][0].start
        != truncated_start_ts.meta.metadata[0][0].start
    )
    assert (
        concatenate_multi_files_ts.meta.metadata[1][0].end
        == truncated_start_ts.meta.metadata[0][0].end
    )
...
def test_truncated_end_ts(concatenate_multi_files_ts, truncated_end_ts):
    ...
    # Now check the truncated (but not truncated out) meta entry
    assert (
        concatenate_multi_files_ts.meta.metadata[-2][0].start
        == truncated_end_ts.meta.metadata[-1][0].start
    )
    assert (
        concatenate_multi_files_ts.meta.metadata[-2][0].end
        != truncated_end_ts.meta.metadata[-1][0].end
    )
...
def test_truncated_both_ts(concatenate_multi_files_ts, truncated_both_ts):
    ...
    assert (
        concatenate_multi_files_ts.meta.metadata[1][0].start
        != truncated_both_ts.meta.metadata[0][0].start
    )
    assert (
        concatenate_multi_files_ts.meta.metadata[1][0].end
        == truncated_both_ts.meta.metadata[0][0].end
    )
    ...
    assert (
        concatenate_multi_files_ts.meta.metadata[-2][0].start
        == truncated_both_ts.meta.metadata[-1][0].start
    )
    assert (
        concatenate_multi_files_ts.meta.metadata[-2][0].end
        != truncated_both_ts.meta.metadata[-1][0].end
    )
...
def test_truncated_outside_tr_ts(truncated_new_tr_all_before_ts, truncated_new_tr_all_after_ts):
    assert (
        truncated_new_tr_all_before_ts.meta.metadata
...
def test_extraction(eve_test_ts, extraction_test_ts):
    ...
    assert (
        eve_test_ts.data.columns[0]
        == eve_test_ts.data.columns[0]
        == list(eve_test_ts.units.keys())[0]
    )
...
def test_concatenation_of_slices(eve_test_ts, concatenated_slices_test_ts):
    ...
    eve_test_ts.meta.metadata[0][2] == concatenated_slices_test_ts.meta.metadata[0][
        2
    ] == concatenated_slices_test_ts.meta.metadata[1][2]
...
def test_concatenation_of_different_data(
    eve_test_ts, fermi_gbm_test_ts, concatenation_different_data_test_ts
):
    ...
    for key in list(concatenation_different_data_test_ts.meta.metadata[0][2].keys()):
        if (
            concatenation_different_data_test_ts.meta.metadata[0][2][key]
            != fermi_gbm_test_ts.meta.metadata[0][2][key]
        ):
            value = False
    for key in list(concatenation_different_data_test_ts.meta.metadata[1][2].keys()):
        if (
            concatenation_different_data_test_ts.meta.metadata[1][2][key]
            != eve_test_ts.meta.metadata[0][2][key]
        ):
            value = False
    assert value
...
def test_column_quantity(eve_test_ts, column_quantity):
    ...
    assert (
        (eve_test_ts.data["CMLon"].values == column_quantity.value)
        | (np.isnan(eve_test_ts.data["CMLon"].values) & np.isnan(column_quantity.value))
    ).all()
...
def test_add_column_from_quantity(eve_test_ts, add_column_from_quantity_ts, column_quantity):
    ...
    assert_quantity_allclose(
        add_column_from_quantity_ts.quantity("quantity_added"), column_quantity
    )
    # Test the full list of columns are pressent
    assert set(add_column_from_quantity_ts.data.columns) == set(eve_test_ts.data.columns) | {
        "quantity_added"
    }
...
def test_add_column_from_array(eve_test_ts, add_column_from_array_ts, column_quantity):
    ...
    # Test the full list of columns are pressent
    assert set(add_column_from_array_ts.data.columns) == set(eve_test_ts.data.columns) | {
        "array_added"
    }
...
def test_fermi_gbm_invalid_peek(fermi_gbm_test_ts):
    ...
    with pytest.raises(ValueError), pytest.warns(
        UserWarning, match="Discarding nonzero nanoseconds"
    ):
        empty_ts.peek()
