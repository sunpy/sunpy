...
def test_append_invalid_timerange(basic_1_md):
    appended = copy.deepcopy(basic_1_md)
    with pytest.raises(ValueError, match="Incorrect datatime or data for append to TimeSeriesMetaData."):
...
def test_find_both_filters(basic_2_md, basic_4_md, complex_append_md):
    assert complex_append_md.find(time='2010-01-02 20:59:57.468999',
...
def test_get_time_filter(complex_append_md):
    assert complex_append_md.get(
        'md1_key1', time='2010-01-01 20:59:57.468999').values() == ['value1']
    assert complex_append_md.get(
        'md2_key2', time='2010-01-02 20:59:57.468999').values() == ['value2']
    assert complex_append_md.get(
        'all_same', time='2010-01-01 20:59:57.468999').values() == ['value3']
    assert complex_append_md.get(
...
def test_get_both_filters(complex_append_md):
    assert complex_append_md.get(
        'all_different', time='2010-01-02 20:59:57.468999', colname='column2').values() == ['diff_2']
...
def test_remove_column(complex_append_md):
    ...
    assert col_removed_md.metadata[1][1] == [
        complex_append_md.metadata[1][1][0], complex_append_md.metadata[1][1][1]]
