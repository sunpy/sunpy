...
class SECCHIBEACONlient(GenericClient):
    ...
    pattern = '{}/{satellite}/secchi/img/{instrument}/{year:4d}{month:2d}{day:2d}/{year:4d}{month:2d}{day:2d}_{}'
    ...
    def get_filenames(self, timerange, telescope, satellite):
        ...
        pattern = ('https://stereo-ssc.nascom.nasa.gov/pub/beacon/{satellite}/{instrument}/img/{telescope}/%Y%m%d/%Y%m%d_%H%M%S_{tele_url}.fts')
        solmon = Scraper(pattern, instrument='secchi', satellite=satellite, telescope=telescope, tele_url=tele_url[telescope])
