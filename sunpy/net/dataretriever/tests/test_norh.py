...
def test_can_handle_query(time):
    ...
    assert ans1 is True
    ans1 = norh.NoRHClient._can_handle_query(time, a.Instrument('norh'), a.Wavelength(10 * u.GHz))
    assert ans1 is True
...
def test_wavelength_range():
    with pytest.raises(ValueError):
        norh.NoRHClient().query(
            a.Time("2016/10/1", "2016/10/2"), a.Instrument('norh'), a.Wavelength(17 * u.GHz, 34 * u.GHz))


def test_query_wrong_wave():
    c = norh.NoRHClient()
    with pytest.raises(ValueError):
        c.query(a.Time("2016/10/1", "2016/10/2"), a.Instrument('norh'), a.Wavelength(50 * u.GHz))


@pytest.mark.online
@pytest.mark.parametrize("time,instrument,wave", [(a.Time('2012/10/4', '2012/10/6'), a.Instrument('norh'), a.Wavelength(
    17 * u.GHz)), (a.Time('2013/10/5', '2013/10/7'), a.Instrument('norh'), a.Wavelength(34 * u.GHz))])
...
@pytest.mark.parametrize("time, instrument, wave",
                         [(a.Time('2012/10/4', '2012/10/6'), a.Instrument('norh'), a.Wavelength(17 * u.GHz)), (a.Time(
                             '2013/10/5', '2013/10/7'), a.Instrument('norh'), a.Wavelength(34 * u.GHz))])
