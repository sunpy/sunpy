...
@mock.patch(
    "sunpy.net.dataretriever.sources.noaa.NOAAIndicesClient.search",
...
def test_fetch(mock_wait, mock_search, mock_enqueue):
    ...
    assert mock_enqueue.called_once_with(
        ("ftp://ftp.swpc.noaa.gov/pub/weekly/RecentIndices.txt", "/some/path/RecentIndices.txt")
    )
...
def test_fido(mock_wait, mock_search, mock_enqueue):
    qr1 = Fido.search(Time("2012/10/4", "2012/10/6"), Instrument("noaa-indices"))
