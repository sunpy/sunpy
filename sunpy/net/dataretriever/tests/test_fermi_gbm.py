...
def test_can_handle_query(time):
    ...
    ans3 = LCClient._can_handle_query(
        time, a.Instrument("gbm"), a.Detector("n5"), a.Resolution("ctime")
    )
    assert ans3 is True
...
@pytest.mark.remote_data
@pytest.mark.parametrize(
    "time,instrument", [(a.Time("2012/8/9", "2012/8/10"), a.Instrument("gbm"))]
)
...
@pytest.mark.remote_data
@pytest.mark.parametrize(
    "time,instrument", [(a.Time("2012/11/27", "2012/11/27"), a.Instrument("gbm"))]
)
...
@pytest.mark.remote_data
@pytest.mark.parametrize(
    "query", [(a.Time("2012/10/4", "2012/10/6") & a.Instrument("gbm") & a.Detector("n5"))]
)
def test_fido(query):
