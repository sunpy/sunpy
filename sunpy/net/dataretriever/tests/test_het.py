...
@pytest.mark.parametrize(
    "timerange, stereo_spacecraft, duration_of_average, url_start,url_end",
...
def test_get_url_for_time_range(timerange, stereo_spacecraft,
                                duration_of_average, url_start, url_end):
    urls = LCClient._get_url_for_timerange(
        timerange,
...
def test_can_handle_query():
    ans1 = stereo.HETClient._can_handle_query(
        Time(TimeRange('2007-03-01', '2010-07-02')),
        Instrument('stereo/het'),
        stereo_spacecraft='ahead',
        duration_of_average=12 * u.h)
    assert ans1 == True
    ans2 = stereo.HETClient._can_handle_query(Time(TimeRange('1988/7/7',
                                                             '2008/7/7')))
    assert ans2 == False
    ans3 = stereo.HETClient._can_handle_query(
        Time(TimeRange('2012/8/9', '2012/8/10')), Instrument('eve'))
    assert ans3 == False


def test_query():
    qr1 = LCClient.query(
        Time(TimeRange('2012/8/9', '2012/10/11')),
        Instrument('stereo/het'),
        stereo_spacecraft='ahead',
        duration_of_average=1 * u.min)
...
@pytest.mark.online
@pytest.mark.parametrize(
    "time, instrument, stereo_spacecraft, duration_of_average",
    [(Time(TimeRange('2012/07/27', '2012/07/28')), Instrument('stereo/het'),
      'ahead', 1 * u.d),
     (Time(TimeRange('2012/10/4', '2012/10/5')), Instrument('stereo/het'),
      'behind', 1 * u.h), ])
def test_get(time, instrument, stereo_spacecraft, duration_of_average):
    qr1 = LCClient.query(time,
                         instrument,
                         stereo_spacecraft=stereo_spacecraft,
                         duration_of_average=duration_of_average)
