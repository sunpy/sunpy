...
@pytest.mark.parametrize(
    ...
    [
        ...
        (attrs.Time('2012/12/8', '2012/12/9') &
         attrs.Instrument('noaa-predict'), "NOAAPredictClient"),
        # Multiple Time Tests
        ((attrs.Time('2012/4/5', '2012/4/5') | attrs.Time('2011/8/7', '2011/8/7')) & attrs.Instrument('norh'), "NoRHClient"),
        ((attrs.Time('2012/7/7', '2012/7/8') | attrs.Time('2011/7/8', '2011/7/9')) & attrs.Instrument('lyra'), "LYRAClient"),
        ((attrs.Time('2012/3/2', '2012/3/3') | attrs.Time('2012/7/8', '2012/7/9')) & attrs.Instrument('eve') &
         attrs.Level(0), "EVEClient")
    ])
def test_offline_client(query, client):
    ...
    # TODO: Add a test here to check the time of the records with the time of
    # the input, will have to be fuzzy
...
@pytest.mark.parametrize("query, client", [
    (attrs.Time('2012/8/8', '2012/8/9') & attrs.Instrument('eve'),
     "VSOClient"),
    ((attrs.Time('2012/3/3', '2012/3/3') | attrs.Time('2013/1/1', '2013/1/1')) & attrs.Instrument('rhessi'), "RHESSIClient"),
])
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (attrs.Time('2011/5/5', '2011/5/5T01:00:00'),
         attrs.Instrument('lyra')),
        (attrs.Time('2012/7/8', '2012/7/8T01:00:00'),
         attrs.Instrument('norh')),
        (attrs.Time('2012/11/27', '2012/11/28'), attrs.Instrument('rhessi')),
        # (attrs.Time('2012/1/8', '2012/3/9'), attrs.Instrument('noaa-indices')),
        (attrs.Time('2012/12/8', '2012/12/9'),
...
@pytest.mark.parametrize("time,instrument1,instrument2", [
    (attrs.Time('2011/3/5', '2011/3/6'), attrs.Instrument('lyra'),
...
def test_multiple_clients(time, instrument1, instrument2):
    ...
    res = Fido.fetch(unifiedresp, wait=False)
    files_downloaded = len(res.wait(progress=False))
...
def test_unifiedresponse_slicing():
    ...
    """
    results = Fido.search(
        attrs.Time("2012/1/1", "2012/1/5"), attrs.Instrument("lyra"))
