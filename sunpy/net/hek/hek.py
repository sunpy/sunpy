...
class HEKClient:
    ...
    The HEK stores solar feature and event data generated by algorithms
    and human observers.
    """
    ...
    default = {"cosec": "2", "cmd": "search", "type": "column", "event_type": "**"}
    ...
    def search(self, *query):
        """
        Retrieves information about HEK records matching the criteria given in
        the query expression.

        If multiple arguments are passed, they are connected with AND.
        The result of a query is a list of unique HEK Response objects
...
class HEKRow(Row):
    ...
    Each HEKRow object is a subclass of `astropy.Table.row`.  The
    column-row key-value pairs correspond to the HEK feature/event
    properties and their values, for that record from the HEK.  Each
    HEKRow object also has extra properties that relate HEK concepts to
    VSO concepts.
    """
    ...
    def get_voevent(self, as_dict=True, base_url="http://www.lmsal.com/hek/her?"):
        """
        Retrieves the VOEvent object associated with a given event and returns
        it as either a Python dictionary or an XML string.
        """
