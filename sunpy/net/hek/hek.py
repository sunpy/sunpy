...
class Response(dict):
    ...
    def get_voevent(self, as_dict=True,
        ...
        if as_dict:
            ...
            # return parse_hek_response_to_quantities(xml_to_dict(response))
        else:
            return response


def parse_hek_response_to_quantities(result):
...
def check_who(data):
    for key, value in data.iteritems():
        if key == "Date" and value == "":
            raise ValueError("KB_ArchivDate not present")
        if key == "Athor" and value == "":
            raise ValueError("KB_Archivist not present")
        if key == "AuthorIVORN" and value == "":
            raise ValueError("KB_ArchivID not present")


def check_why(data):
    for key, value in data.iteritems():
        if key == "lmsal:EVENT_TYPE" and value == "":
            raise ValueError("EVENTTYPE not present")
