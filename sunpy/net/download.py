...
class Downloader(object):
            ...
            with closing(urllib2.urlopen(url)) as sock:
                ...
                with open(fullname, 'wb') as fd:
                    while True:
                        ...
                        if not rec:
                            with self.mutex:
                                self._close(callback, [{'path': fullname}],
                                            server)
                            break
                        else:
                            fd.write(rec)
    ...
    def _attempt_download(self, url, path, callback, errback):
        """ Attempt download. If max. connection limit reached, queue for download
        later.
        """
    ...
    def download(self, url, path=None, callback=None, errback=None):
        ...
        path : function, string
            Location to save file to. Can specify either a directory as a
            string or a function with signature: (path, url).
