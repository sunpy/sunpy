...
def test_jsocresponse_double():
    ...
    assert all(j1.table == astropy.table.vstack(
        [astropy.table.Table(data=[[1, 2, 3, 4]]), astropy.table.Table(data=[[1, 2, 3, 4]])]))
    ...
    payload_expected = {
        'ds':
            '{0}[{1}-{2}]'.format('hmi.M_42s',
                                  start.strftime("%Y.%m.%d_%H:%M:%S_TAI"), end.strftime("%Y.%m.%d_%H:%M:%S_TAI")),
        'format':
            'json', 'method':
                'url', 'notify':
                    '@', 'op':
...
def test_payload_nocompression():
    ...
    payload = client._make_query_payload(start, end, 'hmi.M_42s', compression=None, notify='jsoc@cadair.com')
...
def test_payload_protocol():
    ...
    payload = client._make_query_payload(start, end, 'hmi.M_42s', protocol='as-is', notify='jsoc@cadair.com')
...
def test_query():
    Jresp = client.query(
        attrs.Time('2012/1/1T00:00:00', '2012/1/1T00:01:30'), attrs.Series('hmi.M_45s'), vso_attrs.Sample(
            90 * u.second))
...
def test_post_pass():
    responses = client.query(
        attrs.Time('2012/1/1T00:00:00', '2012/1/1T00:00:45'), attrs.Series('hmi.M_45s'),
        attrs.Notify('jsoc@cadair.com'))
...
def test_post_fail(recwarn):
    res = client.query(
        attrs.Time('2012/1/1T00:00:00', '2012/1/1T00:00:45'), attrs.Series('none'), attrs.Notify('jsoc@cadair.com'))
...
def test_wait_get():
    responses = client.query(
        attrs.Time('2012/1/1T1:00:36', '2012/1/1T01:00:38'), attrs.Series('hmi.M_45s'), attrs.Notify('jsoc@cadair.com'))
...
def test_results_filenames():
    responses = client.query(
        attrs.Time('2014/1/1T1:00:36', '2014/1/1T01:01:38'), attrs.Series('hmi.M_45s'), attrs.Notify('jsoc@cadair.com'))
    path = tempfile.mkdtemp()
