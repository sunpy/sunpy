...
class Scraper:
    ...
    def __init__(self, pattern, regex=False, **kwargs):
        ...
        if not milliseconds:
            ...
            self.now = now.strftime(
                f"{self.pattern[0:milliseconds.start()]}{milliseconds_:03d}{self.pattern[milliseconds.end():]}"
            )
    ...
    def _date_floor(date, timestep):
        ...
        time_tup = [
            orig_time_tup.tm_year,
            orig_time_tup.tm_mon,
            orig_time_tup.tm_mday,
            orig_time_tup.tm_hour,
            orig_time_tup.tm_min,
            orig_time_tup.tm_sec,
        ]
    ...
    def _extractDateURL(self, url):
        ...
        for p, r in zip(pattern_together.split("%")[1:], re_together.split("\\")[1:]):
            if p == "e":
                continue
            regexp = rf"\{r}" if not r.startswith("[") else r
            pattern = f"%{p}"
    ...
    def _ftpfileslist(self, timerange):
        ...
        filesurls = ["ftp://" + f"{urlsplit(url).netloc}{urlsplit(url).path}" for url in filesurls]
    ...
    def _check_timerange(self, url, timerange):
        ...
        """
        if hasattr(self, "extractor") and False:
