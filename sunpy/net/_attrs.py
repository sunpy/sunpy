...
__all__ = ['Physobs', 'Resolution', 'Detector', 'Sample',
           'Level', 'Instrument', 'Wavelength', 'Time', 'Source', 'Provider', 'Info']
...
class Info(SimpleAttr):
    """
    Info[Required] is populated by the VSO only when REST and/or TAP Data Providers are queried
    The string returned in Info[Required] is the Apache session ID generated by the VSO backend,
    and is then returned to the VSO upon requesting file downloads from REST/TAP Data Providers.
    
    For existing VSO SOAP Data Providers Info[Required] is not present at all in the query response
    since it is uneeded in that case.
    """
