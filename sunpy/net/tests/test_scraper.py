...
@pytest.mark.remote_data
def test_ftp():
    pattern = 'ftp://nsro-archive.nro.nao.ac.jp/pub/nsro/norh/data/tcx/%Y/%m/tca%y%m%d'
    s = Scraper(pattern)
...
def test_extract_files_meta():
    baseurl0 = r'ftp://nsro-archive.nro.nao.ac.jp/pub/nsro/norh/data/tcx/%Y/%m/(\w){3}%y%m%d'
