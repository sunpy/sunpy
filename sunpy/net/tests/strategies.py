...
@st.composite
def time_attr(draw, time=datetimes(max_datetime=datetime.datetime.utcnow(),
...
@st.composite
def goes_time(draw, time=datetimes(max_datetime=datetime.datetime.utcnow(),
                                   min_datetime=datetime.datetime(1981,1,1)),
              delta=timedelta()):
    """
    Create an a.Time where it's always positive and doesn't have a massive time
    delta.
...
def range_time(min_date, max_date=datetime.datetime.utcnow()):
    time = datetimes(max_datetime=max_date,
                     min_datetime=datetime.datetime(1960,1,1))
