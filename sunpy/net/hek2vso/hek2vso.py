...
def translate_results_to_query(results):
    ...
    ...             '2011/08/09 07:24:00'), hek.attrs.EventType('FL'))  # doctest: +REMOTE_DATA
    ...
    >>> hek2vso.translate_results_to_query(q)   # doctest: +REMOTE_DATA
    [[<sunpy.net.attrs.Time(2011-08-08 01:30:04.000, 2011-08-10 00:00:04.000)>, ... <sunpy.net.attrs.Wavelength(210.99999999999997, 210.99999999999997, 'Angstrom')>], [<sunpy.net.attrs.Time(2011-08-09 07:22:44.000, 2011-08-09 07:28:56.000)>, <sunpy.net.vso.attrs.Source(SDO: Solar Dynamics Observatory) object at ...>, <sunpy.net.attrs.Instrument(AIA: Atmospheric Imaging Assembly) object at ...>, <sunpy.net.attrs.Wavelength(193.0, 193.0, 'Angstrom')>]]
    """
    queries = []
...
def vso_attribute_parse(phrase):
    ...
    >>> h = HEKClient()  # doctest: +REMOTE_DATA
    >>> h2v = H2VClient()  # doctest: +REMOTE_DATA
    >>> q = h.search(hek.attrs.Time('2011/08/09 07:23:56', '2011/08/09 07:24:00'), hek.attrs.EventType('FL'))  # doctest: +REMOTE_DATA
    >>> len(q)  # doctest: +REMOTE_DATA
    11
...
class H2VClient:
    ...
    def full_query(self, client_query, limit=None, progress=False):
        ...
        >>> h2v = hek2vso.H2VClient()  # doctest: +REMOTE_DATA
        >>> h2v.full_query((hek.attrs.Time('2011/08/09 07:23:56', '2011/08/09 07:24:00'),
        ...                    hek.attrs.EventType('FL')))  # doctest: +SKIP
        """
        self._quick_clean()
        if progress:
