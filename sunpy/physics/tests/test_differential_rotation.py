...
def test_sunpos():
    ...
    assertion = {
        'obliq': (23.4358, Angle, u.deg), 'app_long': (53.3683, Longitude, u.deg), 'dec': (18.6125, Latitude, u.deg),
        'ra': (50.9796, Longitude, u.deg), 'longitude': (53.3705, Longitude, u.deg)
    }
    for k in assertion:
...
def test_calc_P_B0_SD():
    ...
    assertion = {
        'p': (10.4868, Angle, u.deg), 'b0': (-0.8127, Angle, u.deg), 'l0': (0.0000, Angle, u.deg),
        'sd': (16.2364 / 60.0, Angle, u.arcmin)
    }
    for k in assertion:
        np.testing.assert_almost_equal(result[k].to(u.degree).value, assertion[k][0], decimal=4)
...
def test_rot_hpc():
    ...
    x, y = rot_hpc(451.4 * u.arcsec, -108.9 * u.arcsec, '2012-06-15', '2012-06-15 16:05:23')
