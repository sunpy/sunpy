...
def known_displacements_layer_index0():
    return {
        "x": np.asarray([1.136868e-13, -1.048550e01, -2.099555e01]),
...
def known_displacements_layer_index1():
    return {
        "x": np.asarray([1.045963e01, 8.526513e-14, -1.048549e01]),
...
def test_calculate_solar_rotate_shift(
    ...
    assert_allclose(
        test_output["x"].to("arcsec").value,
        known_displacements_layer_index0["x"],
        rtol=5e-2,
        atol=1e-5,
    )
    assert_allclose(
        test_output["y"].to("arcsec").value,
