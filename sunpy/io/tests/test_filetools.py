...
EIT_195_IMAGE = str(Path(testpath).joinpath('EIT/efz20040301.000010_s.fits'))
AIA_171_IMAGE = str(Path(testpath).joinpath('aia_171_level1.fits'))
...
class TestFiletools(object):
    ...
    def test_read_file_fits_gzip(self):
        ...
        for fits_extension in [".fts", ".fit", ".fits"]:
            pair = sunpy.io.read_file(str(Path(sunpy.data.test.rootdir).joinpath("gzip_test{ext}.gz".format(ext=fits_extension))))
    ...
    def test_read_file_jp2(self):
        #Test read jp2
        pair = sunpy.io.read_file(str(Path(sunpy.data.test.rootdir).joinpath(
    ...
    def test_read_file_header_jp2(self):
        # Test jp2
        hlist = sunpy.io.read_file_header(str(Path(sunpy.data.test.rootdir).joinpath(
                                            "2013_06_24__17_31_30_84__SDO_AIA_AIA_193.jp2")))
        assert isinstance(hlist, list)
        assert len(hlist) == 1
        assert isinstance(hlist[0], sunpy.io.header.FileHeader)
    ...
    def test_read_file_ana(self):
        ana_data = sunpy.io.read_file(str(Path(sunpy.data.test.rootdir).joinpath("test_ana.fz")))
    ...
    def test_read_file__header_ana(self):
        ana_data = sunpy.io.read_file_header(str(Path(sunpy.data.test.rootdir).joinpath("test_ana.fz")))
        assert isinstance(ana_data, list)
        assert len(ana_data) == 1
    ...
    def test_write_file_ana(self):
        ana = sunpy.io.read_file(str(Path(sunpy.data.test.rootdir).joinpath("test_ana.fz")))[0]
        sunpy.io.write_file("ana_test_write.fz", ana[0], str(ana[1]))
        assert Path("ana_test_write.fz").exists()
        outpair = sunpy.io.read_file(str(Path(sunpy.data.test.rootdir).joinpath("test_ana.fz")))
