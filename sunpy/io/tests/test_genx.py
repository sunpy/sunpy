...
def test_array_elements_values():
    np.testing.assert_allclose(TESTING["MYSTRUCTURE"]["MYFARRAY"], np.arange(3.0))
    np.testing.assert_allclose(TESTING["MYSTRUCTURE"]["MYFARRAYD"][:, 0], np.arange(6.0, step=2))
    assert TESTING["MYSTRUCTURE"]["MYDARRAYD"][1, 2] == 5.0
    assert TESTING["MYSTRUCTURE"]["NESTEDSTRUCT"]["MYLARRAYD"][3, 0, 1] == 19
    np.testing.assert_allclose(
        TESTING["MYSTRUCTURE"]["NESTEDSTRUCT"]["MYLARRAYD"][2, :, 0], np.arange(12, 17, step=2)
    )
...
@pytest.mark.parametrize(
    "myarray, dtype",
    [
        (TESTING["MYNUMBER_ARRAY"], np.int16),
...
def test_date():
    ...
    assert (
        int("".join(chr(x) for x in TESTING["MYSTRUCTURE"]["RANDOMNUMBERS"][-4:])) == creation.year
    )
