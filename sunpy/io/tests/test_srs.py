...
COORDINATES = [
    {"text": "N10W05", "latitude": 10, "longitude": 5},
...
LONGLAT.add_column(
    MaskedColumn(
        data=[x["longitude"] for x in COORDINATES], name="Longitude", unit=u.deg, mask=True
    )
)
LONGLAT.add_column(
    MaskedColumn(data=[x["latitude"] for x in COORDINATES], name="Latitude", unit=u.deg)
)


@pytest.mark.parametrize(
    "path, number_of_rows",
    [(os.path.join(testpath, elem["file"]), elem["rows"]) for elem in filenames],
)
...
@pytest.mark.parametrize(
    "text, longitude", [(elem["text"], elem["longitude"]) for elem in COORDINATES]
)
...
@pytest.mark.parametrize(
    "text, latitude", [(elem["text"], elem["latitude"]) for elem in COORDINATES]
)
...
@pytest.mark.parametrize(
    "loc_column, exp_longitude, exp_latitude",
