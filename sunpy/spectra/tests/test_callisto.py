...
def test_create_single_glob(CALLISTO_IMAGE, CALLISTO_IMAGE_GLOB_INDEX, CALLISTO_IMAGE_GLOB_KEY):
    ...
    assert_allclose(ca[CALLISTO_IMAGE_GLOB_INDEX].data, CallistoSpectrogram.read(CALLISTO_IMAGE).data)
...
def test_create_glob(CALLISTO_IMAGE_GLOB_KEY):
    PATTERN = os.path.join(os.path.dirname(sunpy.data.test.__file__), CALLISTO_IMAGE_GLOB_KEY)
    ca = CallistoSpectrogram.create(PATTERN)
    assert len(ca) == 2
...
def test_homogenize_factor():
    ...
    c1 = CallistoSpectrogram(a,
                             np.arange(3600),
                             np.array([1]),
                             datetime(2011, 1, 1),
                             datetime(2011, 1, 1, 1), 0, 1, 'Time', 'Frequency', 'Test', None, None, None, False)
    b = 2 * a
    c2 = CallistoSpectrogram(b,
                             np.arange(3600),
                             np.array([1]),
                             datetime(2011, 1, 1),
                             datetime(2011, 1, 1, 1), 0, 1, 'Time', 'Frequency', 'Test', None, None, None, False)
...
def test_homogenize_rightfq():
    ...
    c2 = CallistoSpectrogram(
        ...
        np.arange(3600),
        np.array([0, 1, 2]),
        datetime(2011, 1, 1), datetime(2011, 1, 1, 1), 0, 1, 'Time', 'Frequency', 'Test', None, None, None, False)
    pairs_indices, factors, constants = c1._homogenize_params(c2, 0)
