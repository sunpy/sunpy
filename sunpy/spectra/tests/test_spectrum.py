...
def test_shift_axis():
    ...
    spec.shift_axis(0.5)
    assert np.allclose(spec.freq_axis, [1.5, 2.5, 3.5, 4.5, 5.5, 6.5])
    spec.shift_axis(-0.7)
    assert np.allclose(spec.freq_axis, [0.8, 1.8, 2.8, 3.8, 4.8, 5.8])
...
def test_map_to_axis():
    ...
    assert np.allclose(spec.freq_axis, [0, 1, 4, 9, 16, 25])
