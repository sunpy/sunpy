...
def test_hpc_hpc_sc():
    ...
    sc_in = SkyCoord(
        0 * u.arcsec, 0 * u.arcsec, rsun=rsun, observer=observer_in, frame="helioprojective"
    )
    hpc_out = Helioprojective(observer=observer_out, rsun=rsun)
...
def test_hcrs_hgs():
    ...
    assert quantity_allclose(
        (earth_hgs.lon + 1 * u.deg) % (360 * u.deg), 1 * u.deg, atol=1e-12 * u.deg
    )
...
def test_hgs_hcrs():
    ...
    earth_hgs = SkyCoord(
        0 * u.deg, -5.73 * u.deg, 0.9848139 * u.AU, frame=HeliographicStonyhurst, obstime=obstime
    )
...
def test_hgc_hgc():
    ...
    assert_quantity_allclose(
        new.lon, old.lon - 14.1844 * u.deg, atol=1e-4 * u.deg
    )  # solar rotation
...
def test_hcc_hcc():
    ...
    from_hcc = Heliocentric(
        0.2 * u.AU, 0.3 * u.AU, 0.4 * u.AU, observer=observer, obstime="2001-01-01"
    )
    ...
    from_hcc = Heliocentric(
        0.2 * u.AU, 0.3 * u.AU, 0.4 * u.AU, observer=observer1, obstime="2001-02-01"
    )
...
def test_hcc_hgs_observer_mismatch():
    ...
    hcc1 = Heliocentric(
        0.2 * u.AU, 0.3 * u.AU, 0.4 * u.AU, observer=observer1, obstime=observer1.obstime
    )
    hgs1 = hcc1.transform_to(HeliographicStonyhurst(obstime=hcc1.obstime))

    hcc2 = Heliocentric(
        0.2 * u.AU, 0.3 * u.AU, 0.4 * u.AU, observer=observer2, obstime=observer1.obstime
    )
...
def test_hgs_hcrs_sunspice():
    ...
    old = SkyCoord(
        0 * u.deg, 10 * u.deg, 1 * u.AU, frame=HeliographicStonyhurst(obstime="2019-06-01")
    )
...
def test_hgs_hgc_sunspice():
    ...
    assert_quantity_allclose(
        new.lon, 16.688242 * u.deg + 0.076 * u.deg, atol=1e-2 * u.arcsec, rtol=0
    )
...
def test_hgs_hcc_sunspice():
    ...
    old = SkyCoord(
        CartesianRepresentation([7e5, 8e5, 9e5] * u.km),
...
def test_hpc_hgs_implicit_hcc():
    ...
    explicit1 = start.transform_to(
        Heliocentric(obstime=start.obstime, observer="earth")
    ).transform_to(frame)
    explicit2 = start.transform_to(
        Heliocentric(obstime=frame.obstime, observer="earth")
    ).transform_to(frame)
...
def test_hme_hee_sunspice():
    ...
    old = SkyCoord(
        0 * u.deg, 10 * u.deg, 1 * u.AU, frame=HeliocentricMeanEcliptic(obstime="2019-06-01")
    )
...
def test_hee_hee():
    ...
    obstime = Time("2001-01-01")
    old = SkyCoord(
        90 * u.deg, 10 * u.deg, 1 * u.AU, frame=HeliocentricEarthEcliptic(obstime=obstime)
    )
...
def test_hee_gse_sunspice():
    ...
    old = SkyCoord(
        -20 * u.deg, 10 * u.deg, 0.7 * u.AU, frame=HeliocentricEarthEcliptic(obstime="2019-06-01")
    )
...
def test_gse_gse():
    ...
    old = SkyCoord(
        90 * u.deg, 10 * u.deg, 0.7 * u.AU, frame=GeocentricSolarEcliptic(obstime="2001-01-01")
    )
...
def test_hgs_hci_sunspice():
    ...
    old = SkyCoord(
        120 * u.deg, 10 * u.deg, 1 * u.AU, frame=HeliographicStonyhurst(obstime="2019-06-01")
    )
...
def test_hme_gei_sunspice():
    ...
    old = SkyCoord(
        120 * u.deg, 10 * u.deg, 1 * u.AU, frame=HeliocentricMeanEcliptic(obstime="2019-06-01")
    )
