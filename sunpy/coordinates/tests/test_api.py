...
def test_frame_api():
    ...
    hgic = HelioGraphic(SphericalRepresentation(lon=3*u.hour, lat=10*u.deg))

    # Check the data attribute.
    assert hgic.data.lon == 3*u.hourangle
    ...
    hgic_2 = HelioGraphic(lon=10*u.hour, lat=8*u.deg, distance=1*u.kpc)
    ...
    coord1 = HelioGraphic(lon=0*u.hour, lat=0*u.deg)
    coord2 = HelioGraphic(lon=0*u.hour, lat=1*u.deg)
    assert coord1.separation(coord2).degree == 1.0

    # Now try the same with the 3D separation.
    coord3 = HelioGraphic(lon=0*u.hour, lat=0*u.deg, distance=1*u.kpc)
    coord4 = HelioGraphic(lon=0*u.hour, lat=0*u.deg, distance=2*u.kpc)
    assert coord3.separation_3d(coord4).kpc == 1.0

    # Fail the following example because coord1 and 2 don't have
    # a distance attribute.
    with pytest.raises(ValueError):
        assert coord1.separation_3d(coord2).kpc == 1.0
