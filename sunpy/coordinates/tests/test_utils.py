...
def test_verify_coordinate_helioprojective():
    # Valid Helioprojective coordinates should pass
    hpc_coord = SkyCoord(0*u.arcsec, 0*u.arcsec, frame='helioprojective', observer="earth", obstime="2020-01-01")
    _verify_coordinate_helioprojective(hpc_coord)
    _verify_coordinate_helioprojective(hpc_coord.frame)

    # Non-Helioprojective coordinates should raise
    bad_coord = SkyCoord(0*u.deg, 0*u.deg, frame='heliographic_stonyhurst')
...
def test_solar_angular_radius():
    ...
    # Solar angular radius as seen from Earth is ~960-980 arcsec (varies slightly by time of year)
    sar_value = sar.to(u.arcsec).value
    assert 950 < sar_value < 990
