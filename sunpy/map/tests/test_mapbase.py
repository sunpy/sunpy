...
def test_wcs(aia171_test_map):
    ...
    assert set(wcs.wcs.ctype) == {
        aia171_test_map.coordinate_system.axis1,
        aia171_test_map.coordinate_system.axis2,
    }
...
def test_dsun(generic_map):
    with pytest.warns(
        SunpyUserWarning, match="Missing metadata for observer: assuming Earth-based observer.*"
    ):
...
def test_heliographic_longitude(generic_map):
    with pytest.warns(
        ...
        assert generic_map.heliographic_longitude == 0.0
...
def test_rotation_matrix_pci_j(generic_map):
    np.testing.assert_allclose(generic_map.rotation_matrix, np.array([[0.0, -1.0], [1.0, 0.0]]))


def test_rotation_matrix_crota(aia171_test_map):
    np.testing.assert_allclose(
        aia171_test_map.rotation_matrix,
        np.array([[9.99999943e-01, -3.38820761e-04], [3.38820761e-04, 9.99999943e-01]]),
    )
...
def test_rotation_matrix_cd_cdelt():
    ...
    np.testing.assert_allclose(cd_map.rotation_matrix, np.array([[0.0, -1.0], [1.0, 0]]))
...
def test_rotation_matrix_cd_cdelt_square():
    ...
    cd_map = sunpy.map.Map((data, header))
    np.testing.assert_allclose(cd_map.rotation_matrix, np.array([[0.0, -1], [1.0, 0]]))


def test_swap_cd():
    amap = sunpy.map.Map(os.path.join(testpath, "swap_lv1_20140606_000113.fits"))
    np.testing.assert_allclose(amap.rotation_matrix, np.array([[1.0, 0], [0, 1.0]]))
...
def test_shift_applied(generic_map):
    ...
    crval1 = (
        (
            generic_map.meta.get("crval1") * generic_map.spatial_units[0]
            + shifted_map.shifted_value[0]
        ).to(shifted_map.spatial_units[0])
    ).value
    assert shifted_map.meta.get("crval1") == crval1
    crval2 = (
        (
            generic_map.meta.get("crval2") * generic_map.spatial_units[1]
            + shifted_map.shifted_value[1]
        ).to(shifted_map.spatial_units[1])
    ).value
...
def test_submap(generic_map):
    ...
    submap = generic_map.submap([width / 2.0, height / 2.0] * u.pix, [width, height] * u.pix)

    # Check to see if submap properties were updated properly
    assert submap.reference_pixel.x.value == generic_map.meta["crpix1"] - width / 2.0
    assert submap.reference_pixel.y.value == generic_map.meta["crpix2"] - height / 2.0
    assert submap.data.shape[1] == width / 2.0
    assert submap.data.shape[0] == height / 2.0

    # Check to see if header was updated
    assert submap.meta["naxis1"] == width / 2.0
    assert submap.meta["naxis2"] == height / 2.0
...
def test_resample_metadata(generic_map, sample_method, new_dimensions):
    ...
    resampled_map = generic_map.resample(new_dimensions, method=sample_method)
    assert (
        float(resampled_map.meta["cdelt1"]) / generic_map.meta["cdelt1"]
        == float(generic_map.data.shape[1]) / resampled_map.data.shape[1]
    )
    assert (
        float(resampled_map.meta["cdelt2"]) / generic_map.meta["cdelt2"]
        == float(generic_map.data.shape[0]) / resampled_map.data.shape[0]
    )
    assert resampled_map.meta["crpix1"] == (resampled_map.data.shape[1] + 1) / 2.0
    assert resampled_map.meta["crpix2"] == (resampled_map.data.shape[0] + 1) / 2.0
...
def test_superpixel(aia171_test_map, aia171_test_map_with_mask):
    ...
    assert_quantity_allclose(
        superpixel_map_sum.dimensions[1], aia171_test_map.dimensions[1] / dimensions[1] * u.pix
    )
    assert_quantity_allclose(
        superpixel_map_sum.dimensions[0], aia171_test_map.dimensions[0] / dimensions[0] * u.pix
    )
    assert_quantity_allclose(
        superpixel_map_sum.data[0][0],
        (
            aia171_test_map.data[0][0]
    ...
    assert_quantity_allclose(
        superpixel_map_avg.dimensions[1], aia171_test_map.dimensions[1] / dimensions[1] * u.pix
    )
    assert_quantity_allclose(
        superpixel_map_avg.dimensions[0], aia171_test_map.dimensions[0] / dimensions[0] * u.pix
    )
    assert_quantity_allclose(
        superpixel_map_avg.data[0][0],
        (
            aia171_test_map.data[0][0]
            + aia171_test_map.data[0][1]
            + aia171_test_map.data[1][0]
    ...
    assert_quantity_allclose(
        superpixel_map_sum.mask.shape[0], aia171_test_map.dimensions[1] / dimensions[1]
    )
    assert_quantity_allclose(
        superpixel_map_sum.mask.shape[1], aia171_test_map.dimensions[0] / dimensions[0]
    )
...
def test_rotate(aia171_test_map):
    ...
    np.testing.assert_allclose(rotated_map_1.data[0, 0], 0.0, atol=1e-7)
    np.testing.assert_allclose(rotated_map_2.data[0, 0], 0.0, atol=1e-7)
...
def test_rotate_scale_cdelt(generic_map):
    rot_map = generic_map.rotate(scale=10.0)
    assert rot_map.meta["CDELT1"] == generic_map.meta["CDELT1"] / 10.0
    assert rot_map.meta["CDELT2"] == generic_map.meta["CDELT2"] / 10.0
...
def test_hg_coord(heliographic_test_map):
    ...
    assert isinstance(
        heliographic_test_map.coordinate_frame, sunpy.coordinates.HeliographicCarrington
    )
