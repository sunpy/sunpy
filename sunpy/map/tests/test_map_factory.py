...
a_list_of_many = glob.glob(str(Path(filepath).joinpath("EIT", "*")))
a_fname = a_list_of_many[0]

AIA_171_IMAGE = str(Path(filepath).joinpath('aia_171_level1.fits'))
RHESSI_IMAGE = str(Path(filepath).joinpath('hsi_image_20101016_191218.fits'))
...
class TestMap(object):
    ...
    def test_patterns(self):
        ...
        # Directory
        maps = sunpy.map.Map(str(Path(filepath).joinpath("EIT")))
...
#==============================================================================
    ...
    def test_soho(self):
        ...
        eit = sunpy.map.Map(str(Path(filepath).joinpath("EIT", "efz20040301.000010_s.fits")))
        assert isinstance(eit,sunpy.map.sources.EITMap)

        lasco = sunpy.map.Map(str(Path(filepath).joinpath("lasco_c2_25299383_s.fts")))
        assert isinstance(lasco,sunpy.map.sources.LASCOMap)

        mdi_c = sunpy.map.Map(str(Path(filepath).joinpath("mdi_fd_Ic_6h_01d.5871.0000_s.fits")))
        assert isinstance(mdi_c,sunpy.map.sources.MDIMap)

        mdi_m = sunpy.map.Map(str(Path(filepath).joinpath("mdi_fd_M_96m_01d.5874.0005_s.fits")))
        assert isinstance(mdi_m,sunpy.map.sources.MDIMap)

    def test_stereo(self):
        #Test EUVIMap & CORMap & HIMap
        euvi = sunpy.map.Map(str(Path(filepath).joinpath("euvi_20090615_000900_n4euA_s.fts")))
        assert isinstance(euvi,sunpy.map.sources.EUVIMap)

        cor = sunpy.map.Map(str(Path(filepath).joinpath("cor1_20090615_000500_s4c1A.fts")))
        assert isinstance(cor,sunpy.map.sources.CORMap)

        hi = sunpy.map.Map(str(Path(filepath).joinpath("hi_20110910_114721_s7h2A.fts")))
    ...
    def test_sot(self):
        #Test SOTMap
        sot = sunpy.map.Map(str(Path(filepath).joinpath("FGMG4_20110214_030443.7.fits")))
