...
def test_rotation_angle(map_data, hpc_test_header):
    header = sunpy.map.make_fitswcs_header(map_data, hpc_test_header, rotation_angle=90 * u.deg)
    wcs = WCS(header)
...
def test_rotation_matrix(map_data, hpc_test_header):
    header = sunpy.map.make_fitswcs_header(
        map_data, hpc_test_header, rotation_matrix=np.array([[1, 0], [0, 1]])
    )
    wcs = WCS(header)
...
def test_HGS_CAR_header():
    ...
    new_frame = SkyCoord(
        0 * u.deg, 0 * u.deg, obstime="2019-06-16", frame="heliographic_stonyhurst"
    )
    new_header = sunpy.map.make_fitswcs_header(
        new_data, new_frame, scale=[2.5, 2.5] * u.deg / u.pix, projection_code="CAR"
    )
