"""
Matplotlib-based plotting for `~sunpy.map.GenericMap`.
"""
import copy
import warnings
...
class MapPlotter:
    """
    Handles the plotting of a `~sunpy.map.GenericMap`.
    ...
    def draw_grid(self, axes=None, grid_spacing: u.deg = 15*u.deg, annotate=True,
        ...
        return wcsaxes_compat.wcsaxes_heliographic_overlay(axes,
                                                           ...
                                                           annotate=annotate,
                                                           obstime=self.smap.reference_date,
    ...
    def peek(self, draw_limb=False, draw_grid=False,
        ...
        **matplot_args : dict
        ...
        im = self.smap.plot(axes=axes, **matplot_args)
        ...
        if draw_limb:
            self.smap.draw_limb(axes=axes)

        if draw_grid:
            if grid_spacing is None:
                self.smap.draw_grid(axes=axes)
            else:
                self.smap.draw_grid(axes=axes, grid_spacing=grid_spacing)
