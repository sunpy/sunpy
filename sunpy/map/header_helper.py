...
def make_fitswcs_header(
    ...
    if reference_pixel is None:
        reference_pixel = u.Quantity(
            [(data.shape[1] + 1) / 2.0 * u.pixel, (data.shape[0] + 1) / 2.0 * u.pixel]
        )
    if scale is None:
        scale = [1.0, 1.0] * (u.arcsec / u.pixel)
    ...
    meta_wcs["cdelt1"], meta_wcs["cdelt2"] = (
        scale[0].to_value(meta_wcs["cunit1"] / u.pixel),
    ...
    if rotation_angle is not None:
        ...
        rotation_matrix = np.array(
            [[np.cos(p), -1 * lam * np.sin(p)], [1 / lam * np.sin(p), np.cos(p)]]
        )

    if rotation_matrix is not None:
        (meta_wcs["PC1_1"], meta_wcs["PC1_2"], meta_wcs["PC2_1"], meta_wcs["PC2_2"]) = (
            rotation_matrix[0, 0],
            rotation_matrix[0, 1],
            rotation_matrix[1, 0],
            rotation_matrix[1, 1],
