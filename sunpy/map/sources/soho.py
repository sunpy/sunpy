...
def _dsunAtSoho(date, rad_d, rad_1au=None):
    ...
    since radius_{1au} and radius_{d} are dividing each other we can
    use [arcsec] instead.
...
class EITMap(GenericMap):
    ...
    def _get_mpl_normalizer(self):
        ...
        # Since the data datatype was in fact a np.float32, then the
        # byte-scaling was never picked up.
...
class LASCOMap(GenericMap):
    ...
    def __init__(self, data, header, **kwargs):
        ...
        datestr = "%sT%s" % (self.meta.get('date-obs',
                                           self.meta.get('date_obs')),
                             self.meta.get('time-obs',
                                           self.meta.get('time_obs')))
        self.meta['date-obs'] = datestr

        # If non-standard Keyword is present, correct it for compatibility.
...
class MDIMap(GenericMap):
    ...
    def measurement(self):
        # TODO: This needs to do more than white-light. Should give B, pB, etc.
        if (self.meta['dpc_obsr'].find('Mag') != -1):
            return "magnetogram"
        else:
            return "continuum"
