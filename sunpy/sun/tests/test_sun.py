...
def test_carrington_rotation_number():
    # Tested against values generated by SSWIDL get_sun.pro.  The values were
    # included below are quoted to maximum number of decimal places given by
    # the IDL code.
    assert_quantity_allclose(sun.carrington_rotation_number('2012/11/11'), 2130.2341, atol=1e-3)
    assert_quantity_allclose(sun.carrington_rotation_number('2012/11/11 01:00:00'), 2130.2357, atol=1e-3)
    assert_quantity_allclose(sun.carrington_rotation_number('2013/05/11'), 2136.8702, atol=1e-3)
    assert_quantity_allclose(sun.carrington_rotation_number('2112/07/14 12:34:56'), 3464.9408, atol=1e-3)


def test_carrington_rotation_number_of_central_meridian():
    # Tested against values generated by tim2carr.pro called with the /dc
    # option.  The values were included below are quoted to maximum number of
    # decimal places given by the IDL code.  Note that the value of atol is
    # an order of magnitude larger than the other Carrington rotation number
    # test given above.
    assert_quantity_allclose(sun.carrington_rotation_number_of_central_meridian('2012/11/11'), 2130.2248, atol=1e-2)
    assert_quantity_allclose(sun.carrington_rotation_number_of_central_meridian('2012/11/11 01:00:00'), 2130.2264, atol=1e-2)
    assert_quantity_allclose(sun.carrington_rotation_number_of_central_meridian('2013/05/11'), 2136.8524, atol=1e-2)
    assert_quantity_allclose(sun.carrington_rotation_number_of_central_meridian('2112/07/14 12:34:56'), 3464.9324, atol=1e-2)
