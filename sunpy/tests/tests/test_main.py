...
def test_main_noargs(monkeypatch):
    ...
    assert args in (['-k-online', '-m not figure', 'sunpy'], ['-k-online', '-m not figure', root_dir])
...
def test_main_with_cover(monkeypatch):
    ...
    covpath = os.path.abspath(os.path.join(sunpy.tests.testdir, os.path.join(os.pardir, 'map')))
...
def test_main_with_show_uncovered_lines(monkeypatch):
    ...
    assert args in (['--cov-report', 'term-missing', '-k-online', '-m not figure', os.path.join('sunpy', 'map')],
                    ['--cov-report', 'term-missing', '-k-online', '-m not figure', os.path.join(root_dir, 'map')])
