...
def test_main_noargs(monkeypatch):
    ...
    assert args in (["sunpy", "-m", "not figure"], [root_dir, "-m", "not figure"])
...
def test_main_submodule_map(monkeypatch):
    ...
    assert args in (
        [os.path.join("sunpy", "map"), "-m", "not figure"],
...
def test_main_only_remote_data(monkeypatch):
    ...
    assert args in (
        [os.path.join("sunpy", "map"), "-k remote_data", "--remote-data=any", "-m", "not figure"],
