...
root_dir = str(Path(sunpy.__file__).resolve().parent)
...
def test_main_submodule_map(monkeypatch):
    ...
    assert args in ([str(Path('sunpy').joinpath('map')), '-m', 'not figure'],
                    [str(Path(root_dir).joinpath('map')), '-m', 'not figure'])


def test_main_submodule_jsoc(monkeypatch):
    monkeypatch.setattr(pytest, 'main', lambda args, **kwargs: args)
    args = sunpy.self_test(package='net.jsoc')
    assert args in ([str(Path('sunpy').joinpath('net', 'jsoc')), '-m', 'not figure'],
                    [str(Path(root_dir).joinpath('net', 'jsoc')), '-m', 'not figure'])
...
def test_main_include_remote_data(monkeypatch):
    ...
    assert args in ([str(Path('sunpy').joinpath('map')), '--remote-data=any', '-m', 'not figure'],
                    [str(Path(root_dir).joinpath('map')),'--remote-data=any',  '-m', 'not figure'])


def test_main_only_remote_data(monkeypatch):
    monkeypatch.setattr(pytest, 'main', lambda args, **kwargs: args)
    args = sunpy.self_test(package='map', online_only=True)
    assert args in ([str(Path('sunpy').joinpath('map')), '-k remote_data', '--remote-data=any', '-m', 'not figure'],
                    [str(Path(root_dir).joinpath('map')), '-k remote_data', '--remote-data=any', '-m', 'not figure'])
...
def test_main_coverage(monkeypatch):
    ...
    for a in args:
        assert a in [root_dir, 'sunpy', '--cov', '--cov-config', '-m',
                     'not figure',
                     str(Path(root_dir).joinpath('tests', 'coveragerc')),
                     str(Path('sunpy').joinpath('tests', 'coveragerc'))]
