...
def test_conv_hpc_hcc():
    coord = [40.0, 32.0]
    result = wcs.convert_hpc_hcc(header, img.rsun_arcseconds, coord[0], coord[1])
    assert_array_almost_equal(result, [28748691, 22998953], decimal=3)
 
def test_conv_hcc_hpc():
    coord = [34.0, 132.0]
    result = wcs.convert_hcc_hpc(header, img.rsun_arcseconds, coord[0], coord[1])
...
def test_conv_hg_hpc():
    coord = [34.0, 96.0]
    result = wcs.convert_hg_hpc(header, img.rsun_arcseconds, coord[0], coord[1])
