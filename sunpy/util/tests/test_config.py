...
def test_get_and_create_download_dir():
    ...
    assert path == str(Path.joinpath(USER, 'sunpy', 'data'))
    # test updated config
    new_path = str(Path.joinpath(USER, 'data_here_please'))
    config.set('downloads', 'download_dir', new_path)
    path = get_and_create_download_dir()
    assert path == str(Path.joinpath(USER, new_path))


def test_get_and_create_sample_dir():
    # test default config
    path = get_and_create_sample_dir()
    assert path == str(Path.joinpath(USER, 'sunpy', 'data', 'sample_data'))
    # test updated config
    new_path = str(Path.joinpath(USER, 'data_here_please'))
