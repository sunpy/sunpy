...
def testFilesRange_sameDirectory_local():
    ...
    s = Scraper("/".join(["file:/", rootdir, "EIT", "efz%Y%m%d.%H%M%S_s.fits"]))
...
def testFilesRange_sameDirectory_remote():
    pattern = (
        "http://solarmonitor.org/data/%Y/%m/%d/"
        "fits/{instrument}/"
...
def testFilesRange_sameDirectory_months_remote():
    pattern = (
        "http://www.srl.caltech.edu/{spacecraft}/DATA/{instrument}/" "Ahead/1minute/AeH%y%b.1m"
    )
