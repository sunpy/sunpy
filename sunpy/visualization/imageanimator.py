...
class ImageAnimatorWCS(ImageAnimator):
    ...
    def plot_user_input(self, tick_spacing, x_tick_position=None,
                        y_tick_position=None,  xlabel=None, ylabel=None):
        """Lets the user set the tick spacing and tick labels with proper tick position."""
        ax = self.axes
        if self.unit_x_axis is not None:
            ax.coords[2].set_format_unit(self.unit_x_axis)
        if self.unit_y_axis is not None:
            ax.coords[1].set_format_unit(self.unit_y_axis)
        if not isinstance(tick_spacing, u.Quantity):
            raise TypeError("Astropy Quantities should be provided")
        else:
            ax.coords[2].set_ticks(spacing=tick_spacing, exclude_overlapping=True)
            ax.coords[1].set_ticks(spacing=tick_spacing, exclude_overlapping=True)
        if xlabel is None:
            ax.coords[2].set_axislabel(self.wcs.wcs.ctype[0])
        else:
            ax.coords[2].set_axislabel(xlabel)
        if ylabel is None:
            ax.coords[1].set_axislabel(self.wcs.wcs.ctype[1])
        else:
            ax.coords[1].set_axislabel(ylabel)
        if x_tick_position is not None:
            ax.coords[2].set_axislabel_position(x_tick_position)
        else:
            ax.coords[2].set_axislabel_position('b')
        if y_tick_position is not None:
            ax.coords[1].set_axislabel_position(y_tick_position)
        else:
