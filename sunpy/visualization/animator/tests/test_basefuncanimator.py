...
@pytest.mark.parametrize(
    "fig, colorbar, buttons",
    ((None, False, [[], []]), (plt.figure(), True, [[button_func1], ["hi"]])),
)
def test_base_func_init(fig, colorbar, buttons):
    ...
    tfa._key_press(event)
    assert (
        tfa.sliders[tfa.active_slider]._slider.val == tfa.sliders[tfa.active_slider]._slider.valmax
    )

    event.key = "right"
    tfa._key_press(event)
    assert (
        tfa.sliders[tfa.active_slider]._slider.val == tfa.sliders[tfa.active_slider]._slider.valmin
    )

    event.key = "right"
    tfa._key_press(event)
    assert (
        tfa.sliders[tfa.active_slider]._slider.val
        == tfa.sliders[tfa.active_slider]._slider.valmin + 1
    )
    ...
    tfa._start_play(
        event, tfa.slider_buttons[tfa.active_slider]._button, tfa.sliders[tfa.active_slider]._slider
    )
    assert tfa.timer
...
def test_lineanimator_figure():
    ...
    slider_axis0 = 0
    xdata = np.tile(
        np.linspace(0, 100, (data_shape0[plot_axis0] + 1)), (data_shape0[slider_axis0], 1)
    )
    fig = plt.figure()
