...
def remove_lytaf_events_from_timeseries(
    ts, artifacts=None, return_artifacts=False, lytaf_path=None, force_use_local_lytaf=False
):
    """
...
def _remove_lytaf_events(
    ...
    lytaf = get_lytaf_events(
        time[0], time[-1], lytaf_path=lytaf_path, force_use_local_lytaf=force_use_local_lytaf
    )
    ...
    for suffix in combine_files:
        ...
        for event_row in event_rows:
            ...
            lytaf = np.append(
                ...
                np.array(
                    (
                        Time(datetime.datetime.utcfromtimestamp(event_row[0]), format="datetime"),
                        Time(datetime.datetime.utcfromtimestamp(event_row[1]), format="datetime"),
                        Time(datetime.datetime.utcfromtimestamp(event_row[2]), format="datetime"),
                        Time(datetime.datetime.utcfromtimestamp(event_row[3]), format="datetime"),
...
def _prep_columns(time, channels=None, filecolumns=None):
    ...
    Firstly, this function converts the elements of time, whose entries
    are assumed to be `astropy.time.Time` or `np.ndarray` objects, to
    time strings.  Secondly, it checks whether the number of elements in
    an input list of column names, filecolumns, is equal to the number
    of arrays in the list, channels. If not, a ValueError is raised.  If
    however filecolumns equals None, a filenames list is generated equal
    to ["time", "channel0", "channel1",..., "channelN"] where N is the
    number of arrays in the list, channels (assuming 0-indexed
    counting).
    """
