...
TIME = parse_time(
    np.array([datetime.datetime(2013, 2, 1) + datetime.timedelta(minutes=i) for i in range(120)])
)
...
def test_split_series_using_lytaf():
    ...
    lytaf_tmp = lyra.get_lytaf_events(
        "2010-06-13 02:00", "2010-06-13 06:00", lytaf_path=tmp_dir, combine_files=["ppt"]
    )
...
def test_remove_lytaf_events_from_timeseries(lyra_ts):
    ...
    time, channels, artifact_status_expected = lyra._remove_lytaf_events(
        lyra_ts.data.index,
        channels=[
            np.asanyarray(lyra_ts.data["CHANNEL1"]),
...
def test_remove_lytaf_events_3():
    ...
    with pytest.raises(TypeError):
        lyra._remove_lytaf_events(
            TIME, artifacts=[6], lytaf_path=TEST_DATA_PATH, force_use_local_lytaf=True
        )
    with pytest.raises(ValueError):
        lyra._remove_lytaf_events(
            TIME,
