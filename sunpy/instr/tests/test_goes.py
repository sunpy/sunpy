...
def test_goes_chianti_tem_case2():
    ...
    assert all(temp2 < Quantity([10.25], unit="MK")) and all(temp2 > Quantity([10.24], unit="MK"))
...
def test_goes_chianti_tem_case3():
    ...
    assert all(temp3 < Quantity([11.43], unit="MK")) and all(temp3 > Quantity([11.42], unit="MK"))
...
def test_goes_chianti_tem_case4():
    ...
    assert all(temp4 < Quantity([10.42], unit="MK")) and all(temp4 > Quantity([10.41], unit="MK"))
...
def test_goes_chianti_tem_case5():
    ...
    assert all(temp5 < Quantity(12.30, unit="MK")) and all(temp5 > Quantity(12.29, unit="MK"))
...
def test_goes_chianti_tem_case6():
    ...
    assert all(temp6 < Quantity(11.44, unit="MK")) and all(temp6 > Quantity(11.43, unit="MK"))
...
def test_goes_chianti_tem_case7():
    ...
    assert all(temp7 < Quantity(11.34, unit="MK")) and all(temp7 > Quantity(11.33, unit="MK"))
...
def test_goes_chianti_tem_case8():
    ...
    assert all(temp8 < Quantity(10.36, unit="MK")) and all(temp8 > Quantity(10.35, unit="MK"))
...
def test_calculate_radiative_loss_rate():
    ...
    exp_data = np.array([1.78100055e19, 1.66003113e19, 1.71993065e19, 1.60171768e19, 1.71993065e19])
    np.testing.assert_allclose(goeslc_test.data.rad_loss_rate[:5], exp_data)
...
def test_calc_rad_loss_nokwags():
    ...
    rad_loss_expected = {"rad_loss_rate": 3.01851392e19 * Quantity(np.ones(2), unit="J/s")}
...
def test_calc_rad_loss_obstime():
    ...
    rad_loss_expected = {
        ...
        "rad_loss_cumul": Quantity(
            [6.03702783e19, 1.20740557e20, 1.81110835e20, 2.41481113e20, 3.01851392e20], unit="J"
        ),
    }
    assert sorted(rad_loss_test.keys()) == sorted(rad_loss_expected.keys())
    assert_quantity_allclose(
    ...
    assert_quantity_allclose(
        rad_loss_test["rad_loss_cumul"], rad_loss_expected["rad_loss_cumul"], rtol=0.0001
    )
...
def test_goes_lx_obstime():
    ...
    lx_expected = {
        ...
        "longlum_cumul": Quantity(
            [3.93721131e18, 7.87442262e18, 1.18116339e19, 1.57488452e19, 1.96860565e19], unit="J"
        ),
        "shortlum_cumul": Quantity(
            [3.93721131e17, 7.87442262e17, 1.18116339e18, 1.57488452e18, 1.96860565e18], unit="J"
        ),
    }
    assert sorted(lx_test.keys()) == sorted(lx_expected.keys())
    assert_quantity_allclose(lx_test["longlum"], lx_expected["longlum"], rtol=0.1)
