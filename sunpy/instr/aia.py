...
def aiaprep(aiamap):
    ...
    `~sunpy.map.sources.sdo.AIAMap`. Rotates, scales and translates the image
    so that solar North is aligned with the y axis, each pixel is 0.6 arcsec
    across, and the center of the sun is at the center of the image. The actual
    transformation is done by Map's.
    ...
    if (aiamap.scale[0] / 0.6).round() != 1.0 * u.arcsec / u.pix and aiamap.data.shape != (
        4096,
        4096,
    ):
        ...
        scale = (
            0.6 * u.arcsec
        )  # pragma: no cover # can't test this because it needs a full res image
    ...
    newmap = tempmap.submap(
        u.Quantity([range_side[0], range_side[0]]), u.Quantity([range_side[1], range_side[1]])
    )
