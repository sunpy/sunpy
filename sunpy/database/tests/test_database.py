...
def query_result():
    return vso.VSOClient().search(
        vso.attrs.Time("20130801T200000", "20130801T200030"), vso.attrs.Instrument("PLASTIC")
    )
...
def download_qr():
    return vso.VSOClient().search(
        vso.attrs.Time("2012-03-29", "2012-03-29"), vso.attrs.Instrument("AIA")
    )
...
def num_entries_from_vso_query(db, query, path=None, file_pattern="", overwrite=False):
    db.download_from_vso_query_result(query, path=path, overwrite=overwrite)
...
def test_vso_query_block_caching(database, download_qr, tmpdir):
    ...
    num_of_fits_headers = num_entries_from_vso_query(
        database,
        download_qr,
    ...
    num_of_fits_headers_1 = num_entries_from_vso_query(
        database,
        download_qr[:1],
    ...
    num_of_fits_headers_2 = num_entries_from_vso_query(
        database,
        download_qr,
...
@pytest.mark.remote_data
def test_vso_query_block_caching_with_overwrite_true_flag(database, download_qr, tmpdir):
...
def test_add_fom_path_duplicates(database):
    with pytest.warns(AstropyUserWarning, match="File may have been truncated"):
        database.add_from_dir(waveunitdir)
    assert len(database) == 4
    with pytest.raises(EntryAlreadyAddedError), pytest.warns(
        AstropyUserWarning, match="File may have been truncated"
    ):
        database.add_from_dir(waveunitdir)
...
def test_query(filled_database):
    ...
    bar = Tag("bar")
    bar.id = 2
    entries = filled_database.search(attrs.Tag("foo") | attrs.Tag("bar"), sortby="id")
...
def test_fetch(database, download_query, tmpdir):
    ...
    database.fetch(*download_query, path=str(tmpdir.join("{file}.fits")), progress=True)
    fits_pattern = str(tmpdir.join("*.fits"))
    num_of_fits_headers = sum(len(fits.get_header(file)) for file in glob.glob(fits_pattern))
    assert len(database) == num_of_fits_headers
    for entry in database:
...
def test_fetch_separate_filenames():
    ...
    assert os.path.isfile(
        os.path.join(tmp_test_dir, "aia_lev1_335a_2012_08_05t00_00_02_62z_image_lev1.fits")
    )
    assert os.path.isfile(
        os.path.join(tmp_test_dir, "aia_lev1_94a_2012_08_05t00_00_01_12z_image_lev1.fits")
    )
...
def test_split_database(split_function_database, database):
    ...
    observed_source_entries = split_function_database.search(vso.attrs.Provider("xyz"), sortby="id")
