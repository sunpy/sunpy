...
RHESSI_IMAGE = str(Path(testdir).joinpath('hsi_image_20101016_191218.fits'))
EIT_195_IMAGE = str(Path(testdir).joinpath('EIT/efz20040301.000010_s.fits'))
GOES_DATA = str(Path(testdir).joinpath('go1520110607.fits'))
...
def test_entries_from_dir():
    ...
    for entry, filename in entries:
        if filename.endswith('na120701.091058.fits'):
            break
    assert entry.path in (str(Path(waveunitdir).joinpath(filename)), filename)
...
def test_create_display_table():
    ...
    with open(Path(filedir).joinpath('test_table.txt'), 'r') as f:
        stored_table = f.read()
