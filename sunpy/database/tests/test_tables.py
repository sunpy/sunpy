...
def qr_with_none_waves():
    return vso.VSOClient().search(
        ...
        vso.attrs.Provider("SDAC"),
        vso.attrs.Instrument("VIRGO"),
    )
...
def qr_block_with_missing_physobs():
    return vso.VSOClient().search(
        ...
        vso.attrs.Provider("SSC"),
        vso.attrs.Wavelength(10 * u.kHz, 160 * u.kHz),
    )[0]
...
def qr_block_with_kev_unit():
    return vso.VSOClient().search(
        vso.attrs.Time((2011, 9, 20, 1), (2011, 9, 20, 2)), vso.attrs.Instrument("RHESSI")
    )[0]
...
def test_entries_from_fido_search_result(fido_search_result):
    ...
    assert entries[60] == DatabaseEntry(
        ...
        physobs="",
        fileid=(
            "ftp://anonymous:data@sunpy.org@solar-pub.nao.ac.jp/"
...
def test_entries_from_dir_recursively_true():
    with pytest.warns(AstropyUserWarning, match="File may have been truncated"):
        entries = list(
            entries_from_dir(
                testdir, True, default_waveunit="angstrom", time_string_parse_format="%d/%m/%Y"
            )
        )
...
def test_entries_from_dir_recursively_false():
    with pytest.warns(AstropyUserWarning, match="File may have been truncated"):
        entries = list(
            entries_from_dir(
                testdir, False, default_waveunit="angstrom", time_string_parse_format="%d/%m/%Y"
            )
        )
