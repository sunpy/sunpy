...
def test_walker_create_starred_true(session):
    ...
    assert entries == [
        ...
        tables.DatabaseEntry(
            id=6, path="/tmp", starred=True, download_time=datetime(2005, 6, 15, 6)
        ),
...
def test_walker_create_ored_query(session):
    ...
    fits_header_entry.id = 1
    assert (
        tables.DatabaseEntry(id=2, starred=True, download_time=datetime(2005, 6, 15, 2)) in entries
    )
    assert (
        tables.DatabaseEntry(id=4, starred=True, download_time=datetime(2005, 6, 15, 4)) in entries
    )
    ...
    assert (
        tables.DatabaseEntry(id=8, starred=True, download_time=datetime(2005, 6, 15, 8)) in entries
    )
    assert (
        tables.DatabaseEntry(
            id=10,
...
def test_walker_create_complex_query(session):
    ...
    assert (
        tables.DatabaseEntry(id=3, path="/tmp", download_time=datetime(2005, 6, 15, 3)) in entries
    )
    assert tables.DatabaseEntry(id=7, download_time=datetime(2005, 6, 15, 7)) in entries
    assert (
        tables.DatabaseEntry(id=9, path="/tmp", download_time=datetime(2005, 6, 15, 9)) in entries
    )
    assert (
        tables.DatabaseEntry(
            id=10,
...
def test_walker_create_downloadtime_inverted(session):
    ...
    assert entries == [
        ...
        tables.DatabaseEntry(id=5, tags=[tag], download_time=datetime(2005, 6, 15, 5)),
        tables.DatabaseEntry(
            id=6, starred=True, path="/tmp", download_time=datetime(2005, 6, 15, 6)
        ),
        tables.DatabaseEntry(
            id=10,
            starred=True,
