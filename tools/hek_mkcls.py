...
code-generation in lieu of dynamic magic is that code-generation ensures that
tools (e.g. IPython) are able to automatically complete names of members.
Considering the sheer amount of members it is essential that users are able to
use completion.
...
NAMES = defaultdict(lambda: None, {"CME": "CE"})
...
def mk_cls(key, used, pad=1, nokeys=True, init=True, name=None, base="EventType"):
    ...
    keys = sorted([(k, v) for k, v in fields.iteritems() if k.startswith(key)])
    used.update({k for k, v in keys})
    if not keys:
        if not nokeys:
            raise ValueError
        return "{} = EventType({!r})".format(key, name.lower())
    ret = ""
    ret += "@apply\nclass {}({}):\n".format(name, base)
    for k, v in keys:
...
if __name__ == "__main__":
    ...
    dest = (
        os.path.join(os.path.dirname(__file__), os.pardir, "sunpy", "net", "hek", "attrs.py")
        if len(sys.argv) <= 1
        else sys.argv[1]
    )
